...
if($_SERVER['REQUEST_METHOD'] == "POST"){
    ...
    if(empty($username) && empty($email) &&empty($password) && empty($cpassword) && empty($phonenumber)){
       ...
       elseif(strlen($password) < 6) {
        ...
        $query="SELECT * FROM admin_form WHERE email='$email'";
        $result =mysqli_query($con,$query);
        if(mysqli_num_rows($result) > 0) {
            echo "<script>alert('users already exist');</script>";
        }
        else {
         $sql ="INSERT INTO admin_form(username,email,password,number)VALUES('$username','$email','$hash_password','$number')";
            ...
            <div class="row">
                <div class="col-lg-10">
                   ...
                   <p style="color:black ;"> Already have an account?</p>
                    <a href="signin.php" style="text-decoration: none; color: black;" class="btn btn-outline-sucess mb-2">Login now</a>
